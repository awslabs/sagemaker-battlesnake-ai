#/usr/bin/env python
#
# Based on rollout.py example from the Ray RLlib project
...
print("\n\n\nNOTE: this script will likely fail/timeout if you are currently connected to a VPN!\n\n")
...
def create_parser(parser_creator=None):
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="Number of agents in the Battlesnake environment")
    parser.add_argument(
        "--map-size",
        default=21,
        type=int,
        help="Battlesnake map height/width")
...
def run(args, parser):
    ...
    config['explore'] = False  # disable exploration for evaluation purposes only
...
def rollout(agent,
    ...
    while keep_going(steps, num_steps, episodes, num_episodes):
        ...
        # SPERRY
        agent_total_rewards =  { 'agent_0': 0, 'agent_1': 0, 'agent_2': 0, 'agent_3': 0, 'agent_4': 0 }
...
# SPERRY
                    agent_total_rewards[agent_id] += r
            ...
            epsteps += 1
            obs = next_obs
            dump_array.append(env.env._get_state().tolist())
        saver.end_rollout()

        winner = max(agent_total_rewards, key=agent_total_rewards.get)

        print("Episode #{}: reward: {}  steps: {}  winner: {}  totals: {} ".format(episodes, reward_total, epsteps, winner, agent_total_rewards))
#SPERRY
        outfile = "outfile_%03d.json" % (episodes)
        with open(outfile, 'w', encoding='utf-8') as f:
            json.dump(dump_array, f, ensure_ascii=False, indent=None)
...
if __name__ == "__main__":
    ...
    def env_battlesnake(env_config):
        return MultiAgentBattlesnake(num_agents=args.num_agents, map_height=args.map_size)

    register_env("MultiAgentBattlesnake-v1", env_battlesnake)
